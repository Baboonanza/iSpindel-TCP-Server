    ...
    else
    {
        ...
        chart = new Highcharts.Chart(
        {
            ...
            yAxis: [
                {
                    ...
                    labels:
                    {
                        ...
                        formatter: function()
                        {
                            return 1 + (this.value / 1000 ) + first_y_unit
                        }
                    },
			...
			const pointData = chartDens.find(row => row.timestamp === this.point.x)
                        return '<b>' + recipe_name + ' </b>'+pointData.recipe+'<br>'+'<b>'+ this.series.name + ' </b>' + tooltip_at + ' ' + Highcharts.dateFormat('%d.%m %H:%M', new Date(this.x))  + ' ' + tooltip_time + ' ' + (1 + (this.y / 1000)).toFixed(3) + first_y_unit;
                    }
                }
            },  
            legend: 
            ...
            [
                {
                    ...
                    },
                   {
                    ...
                    name: first_y,
                    color: '#FF0000',
                    data: chartDens.map(row => [row.timestamp, ( row.value - 1 ) * 1000, row.text]),
                    marker: 
                    {
                        symbol: 'square',
                        enabled: false,
