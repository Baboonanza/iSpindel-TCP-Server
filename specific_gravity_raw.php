...
$first_y_min = 0; #intval(get_settings_from_sql($conn,"DIAGRAM","GLOBAL",$PARA_FIRST_Y_MIN));
$first_y_max = 25; #intval(get_settings_from_sql($conn,"DIAGRAM","GLOBAL",$PARA_FIRST_Y_MAX));
            ...
            tooltip:
            {
                ...
                formatter: function() 
                {
                    if(this.series.name == second_y) {
                        ...
                        return '<b>' + recipe_name + ' </b>'+pointData.recipe+'<br>'+'<b>'+ this.series.name + ' </b>' + tooltip_at + ' ' + Highcharts.dateFormat('%d.%m %H:%M', new Date(this.x))  + ' ' + tooltip_time + ' ' + this.y.toFixed(3) + second_y_unit;
                    }
                }
            },  
            legend: 
